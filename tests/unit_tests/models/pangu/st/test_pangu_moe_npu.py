...
def test_pangu_moe_paths_on_npu(npu_device):
    ...
    block.gate = SimpleNamespace(weight=torch.nn.Parameter(torch.randn((1, 1), device=npu_device)))

    def _prefill(hidden_states, residual, attn_metadata):
        assert hidden_states.device == npu_device
        return torch.zeros_like(hidden_states)

    def _decode(hidden_states, residual, attn_metadata, layer_id, next_attention_weights=None):
        assert hidden_states.device == npu_device
    ...
    prefill_meta = _metadata(object())
    decode_meta = _metadata(pangu_mod.AscendAttentionState.DecodeOnly)

    out_prefill = block.forward(hidden_states, residual, prefill_meta, layer_id=0, is_hybrid_chunked_prefill_graph_mode=False)
    assert torch.all(out_prefill == 0)
    assert block.is_init_gate

    out_decode = block.forward(hidden_states, residual, decode_meta, layer_id=0, is_hybrid_chunked_prefill_graph_mode=False)
    assert torch.all(out_decode == 1)

    out_hybrid = block.forward(hidden_states, residual, prefill_meta, layer_id=0, is_hybrid_chunked_prefill_graph_mode=True)
