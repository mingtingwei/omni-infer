...
def prepare_ranktables(prefill_group: ServerGroup, decode_group: ServerGroup, p_ranks: int, d_ranks: int):
    ...
    if prefill_server_ip != decode_server_ip:  # on multiple machines
        ...
        if p_device.device_port is not None:
            p_devices["device"][0]["device_port"] = str(p_device.device_port)
        d_devices = {
            "device": [
                {
        ...
        if d_device.device_port is not None:
            d_devices["device"][0]["device_port"] = str(d_device.device_port)
        ...
        if p_device.device_port is not None:
            pd_devices["device"][0]["device_port"] = str(p_device.device_port)
        if d_device.device_port is not None:
            pd_devices["device"][1]["device_port"] = str(d_device.device_port)
